...
{
    // Primeiro encontra o menor valor não nulo nas células usadas
    ...
    for (int i = colmeia->numeroFavos - 1; i >= 0; i--)
    {
        ...
        for (int j = colmeia->favos[i].numeroCelulas - 1; j >= 0; j--)
        {
            ...
            if (celula->tipo == tipoAlimento && 
                ...
                celula->conteudo > 0)
            {
                // Encontrou valor não nulo menor
    ...
    // Se não encontrou nenhuma célula com conteúdo
    if (conteudoMinimo == 999999)
        return NULL;

    // Agora retorna a primeira célula (mais nova) que tem esse valor mínimo
    for (int i = colmeia->numeroFavos - 1; i >= 0; i--)
    {
        for (int j = colmeia->favos[i].numeroCelulas - 1; j >= 0; j--)
        {
   ...
   while (relatorioPendente > 0 && !terminado)
   {
       ...
       for (int m = tempoAtual + 1; m <= tempoAlvoRelatorio && !terminado; m++)
       {
           ...
           if (temporadas > 0)
           {
               ...
               // Acumula trabalho (só se a abelha não precisar comer agora)
               for (int i = 0; i < colmeia->numeroAbelhas; i++)
               {
                   if (colmeia->abelhas[i].tipo != -1 && 
               ...
               // Tenta recolher pólen antes da alimentação
               recolherPolen(colmeia, &trabalhoForrageiras, &trabalhoFaxineiras);
               // Depois tenta por ovos
           ...
           // Alimentação - abelhas comem em momentos diferentes
           for (int i = 0; i < colmeia->numeroAbelhas; i++)
           {
               if (colmeia->abelhas[i].tipo != -1 &&
                   (colmeia->abelhas[i].tempoNascimento == m || // Recém-nascida
                    m - colmeia->abelhas[i].tempoUltimaRefeicao >= 1440)) // 24h sem comer
               {
                   // Tenta encontrar mel e pólen
                   ...
                   if (!celulaMel || celulaMel->conteudo < 10)
                   {
                       ...
                       // Todo mel fica vazio quando acaba
                       for (int k = 0; k < colmeia->numeroFavos; k++)
                       {
                           for (int l = 0; l < colmeia->favos[k].numeroCelulas; l++)
                           {
                               if (colmeia->favos[k].celulas[l].tipo == MEL)
                               {
                                   colmeia->favos[k].celulas[l].estado = VAZIA;
                                   colmeia->favos[k].celulas[l].conteudo = 0;
                               }
                           }
                       }
                   }
                   else
                   {
                       ...
                       else
                       {
                           ...
                           // Pólen sempre permanece em USADA pois está sendo reposto
                           
                           colmeia->abelhas[i].tempoUltimaRefeicao = m;
                       }
                   }
               }
           }
